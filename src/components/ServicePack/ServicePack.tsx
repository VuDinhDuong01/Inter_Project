...
export const ServicePack = () => {
    ...
    return (
        <div className="lg:mb-[164px] 2xl:max-w-[1232px] 2xl:m-auto 2xl:mb-[164px] lg:px-[108px] 2xl:px-0 lg:bg-transparent bg-[#FFFCF0] lg:py-0 py-[50px] ">
                ...
                <Slick slidesToShow={3}>{DataService.map((item, index) => {
                    ...
                    return <div key={index} className='mb-[44px]'>
                        <div style={{ boxShadow: '4px 2px 15px 0px rgba(0, 0, 0, 0.05)' }} className={` ${item.title === 'Chuyên nghiệp' || item.title === 'Tổ chức' ? 'bg-[#F1F1F1]' : 'bg-white'} mx-[16px]  rounded-[20px] h-[424px] sm:h-[440px]   px-[24px]`}>
                           ...
                           <Button className={`flex rounded-[16px] lg:w-[275px] 2xl:w-[270px] w-[200px] h-[48px] text-center justify-center items-center text-white font-FontSan text-[24px] font-[600] mb-[37px]  ${item.title === 'Chuyên nghiệp' || item.title === 'Tổ chức' ? 'bg-[#7A7A7A]' : 'bg-[#008345]'}   m-auto`}>{t('HomePage.begin')}</Button>
                           </div>
